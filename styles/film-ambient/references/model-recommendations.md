...
六个槽中最多同时换两个。**光型和机位一次只调一个**，
一次全换等于重新抽卡。

**胶片型号（`--film`）是性价比最高的锚点** —— 一个型号名同时锁定色彩倾向、
宽容度、高光行为与颗粒粗细，比写一堆"胶片质感 / film grain"管用得多。
默认 `pro400h`（日系青绿通透）；夜戏换 `cinestill800t`；暖调换 `portra400`。
型号与光型的搭配见 `light-patterns.md` 组合规则第 4 条。
