...
**这个 skill 不是提示词库。** 出图质量的决定性因素不是"选哪套参数"，
而是"有没有想清楚要什么"。所以每次出图前，先走一遍 [`AESTHETIC.md`](./AESTHETIC.md)：

1. 读 §2 **六条第一性原理**（克制 / 暗示 / 光有出处 / 情绪浓度 / 不完美 / 对比度预算）
2. 走 §3 **推导链**：情绪 → 光 → 色 → 构图 → 质感 → 镜头（顺序不能乱，后一步是前一步的函数）
3. 答完 §6 **五问**：情绪是什么 / 光从哪来 / 删掉了什么 / 哪处不完美 / 记忆点是什么
4. **答不出任何一条就不要生成** —— 先去问用户，或先想清楚
5. 出图不理想时走 §4 **诊断回路**：定位失败维度 → 只改那一维 → 重出。**不要靠抽卡**

`AESTHETIC.md` 回答"我应该要什么"；`styles/` 下的槽位、预设、视觉定义回答"怎么实现"。
**顺序反了（先翻预设再倒推情绪），就把这个 skill 用成了提示词库 —— 那是复制，不是创作。**

## 维度一：画法（必选）

| 风格 | 对标作品 / 质感来源 | 特征 | 目录 |
|---|---|---|---|
| **`3d-realistic`** 国漫 3D 写实 | 国漫电影级 CGI 的工业水准（不抄仙侠荧光） | **70% 真人 + 30% 理想化**、成年少女骨相、SSS 皮肤与微毛孔、PBR/RT/GI、发饰克制不堆砌 | `styles/3d-realistic/` |
...
### Step 0 — 审美判断（先做这步，不能跳）

读 [`AESTHETIC.md`](./AESTHETIC.md)，按 §3 推导链定下
**情绪 → 光 → 色 → 构图 → 质感 → 镜头**，并答完 §6 五问。
用户描述模糊时（"画个古风美人"），用 §6 的问题**去问用户**，而不是直接开抽。
这一步决定了出图上限；后面所有参数都只是它的实现。
...
| 文件 | 内容 |
|---|---|
| **`AESTHETIC.md`** | **审美判断层 —— 先读这份。** 六条第一性原理（克制 / 暗示 / 光有出处 / 情绪浓度 / 不完美 / 对比度预算）+ 推导链 + 诊断回路 + 反例解剖 + 生成前 5 问 |
