...
**审查并修正 `ink-wash`** —— 它是三套画法里唯一没按新标准体检过的。

### 背景（Why）

`ink-wash` 的固定层只讲媒介（笔触、宣纸、留白），**完全不讲人物** —— 和旧版
`3d-realistic` 同样的漏洞：人物长相交给模型自由发挥，可能出幼态/网红脸。
另外它的资产里有**男性**（白衣书生），与「古风美人 · 只画女性」的定位不符。

### 变更（Changed）

- **删除男性内容**：`assets/character/scholar.jpg`、`examples/character/scholar.md`，
  以及 README 作品区的「白衣书生」图与 CLI 示例里的书生
- **分类统一**：`character` → `character-female`（与 `3d-realistic` 一致，agent 不用记两套）
- 清朝代文档里的男性主语：`dynasties/wei-jin` 的「魏晋名士」→「魏晋仕女」，
  README / SKILL 的「名士、洛神」→「洛神、仕女」，「名士风骨」→「魏晋风骨」

### 新增（Added）

- **固定层加入「古典仕女」审美签名**：成年东方女性、清雅端庄、鹅蛋脸、
  **五官以简练笔触写意（不刻画过度）**、姿态含蓄（侧身/回眸/执扇/垂眸）、
  衣纹线条流畅飘逸 —— 并明确 **不幼态、不网红、不浓妆**
- **负面词补两组**：幼态组（幼态 / 娃娃脸 / 童颜 / 萌系）与妆造组（网红脸 / 网红妆 / 浓妆 /
  过度刻画五官 / 写实五官）
- **`visual-dna.md` 新增 §0：本画法在通用审美层里的权重** —— 明确 `AESTHETIC.md` 的六条
  在水墨里不是平权的：

  | 原理 | 权重 |
  |---|---|
  | §2.2 暗示优于直给 | **最高**（留白是构图的一部分） |
  | §2.1 克制法则 | **最高**（墨分五色就够，多一笔就俗） |
  | §2.3 光必须有出处 | 不适用（水墨没有光源，靠墨色浓淡） |
  | §2.5 真实感来自不完美 | **反转**（水墨要笔意，不要物理真实） |
  | §2.6 对比度预算 | 转化为「墨色浓淡」，朱砂/金只作点睛 |

### 诊断口诀

> **「像国画」靠留白与笔意，「像美人」靠姿态与点睛色 —— 两者都不靠把脸画细。**

---

## v3.7.0（2026-09-15）

**定位收窄为「古风美人」——只画女性**；并修复 `3d-realistic` 女性固定层的幼态泄漏。

### 背景（Why）

这个 skill 是画**古风美人**的，不该出现在男性角色路径。上一版 `3d-realistic` 保留了
`character-male` 分类与两个男性预设（`swordsman-night` / `sect-master`），与定位不符。
同时首轮实测暴露：女性固定层仍会**滑向幼态**（脸偏娃娃、腮红过重、嘴唇油亮）。

### 变更（Changed）

- **删除男性路径**：`MALE_FIXED_*`、`character-male` 分类、男性预设、男性资产
  （`assets/character-male/`、`examples/character-male/`、`examples-zh/.../male-characters.md`），
  以及 `prompt-library-zh.md` 的「4.1 男性角色」「4.3 战斗/动态角色」两节
- `SKILL.md` / `README.md` / 路由说明 / `CONTRIBUTING.md`：定位改为「只画女性 · 古风美人」
- `3d-realistic` 命名风格 6 → **4**（全部女性）

### 修复（Fixed）

女性固定层的**幼态泄漏**（首轮实测：脸偏娃娃、腮红过重、唇油亮），逐条归因后改：

| 病灶 | 原措辞（往幼态拉） | 改为 |
|---|---|---|
| 骨相 | "略带柔和感但保持成年少女骨相" | "骨相与比例**明确是成年人的（不是儿童比例）**，下颌结构清晰" |
| 眼睛 | "**大**而清澈的杏眼" | "与脸型**比例协调、不过大**" |
| 腮红 | "脸颊自然**淡粉红晕**" | "只有**极淡**的自然血色（**不是浓重腮红**）" |
| 嘴唇 | "柔软**饱满**的樱粉唇" | "自然不夸张，**不油亮、不饱满**" |
| 负面词 | — | 补「幼态妆组」：娃娃脸 / 童颜 / 幼态 / 浓重腮红 / 油亮唇 / 玻尿酸唇 / 滤镜脸 / 眼睛过大 |

### 已知问题（Known Issues）

- **垫图会覆盖画幅**：实测用 1672×941 横图做 `--ref` 时，`--ratio 3:4` 被带成横图。
  规避：垫图尽量选与目标画幅一致（3:4）的图。待后续在 `generate.py` 层做画幅校验。
- **幼态修复有效但未完全达标**：二轮实测腮红与唇明显改善，骨相仍偏年轻，需继续收紧措辞。
